...
inRedir - Subclass of base. When execute() is called, instead of executing, simply changes the fd 0 (stdin) to a given file. Throws an error if this file is missing.

outSingle and outDouble - Subclass of base. When execute() is called, instead of executing a command, simply changes the fd 1 (stdout) to a given file. The difference between the two files is that, if outDouble finds that the specified output file exists already, it will append the output to the file instead of replacing it fully.

piping - Subclass of base. Signifies that there is a pipe connector, and links the previous and next command in the vector list to allow them to communicate.
...
### Diagram
![alternative text](https://github.com/cs100/assignment-sigp/blob/master/images/OMT%20Diagram.png)
...
- The inRedir, outSingle, outDouble, and piping classes are mysteriously empty.
